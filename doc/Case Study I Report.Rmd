...
In tab 1, map views for the number of new cases, new deaths and people fully vaccinated are presented. Users can select continents they are interested in the sidebar panel to zoom in the map. We plotted the number of new cases and the number of new deaths together because we found that these two variables had high correlation in a general sense. And we also plotted the number of people fully vaccinated below and we found this variable increased rapidly after a certain point in time. Below the three maps, a table for selected variable in the selected continent is presented. Users can view this table in ascending order or decreasing order, and can also search for a certain country in the selected continent. All in all, tab 1 generally shows some variables of covid-19 geographically and users can play with it to view whatever they are interested in.
